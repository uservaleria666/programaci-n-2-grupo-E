...
#Cargar desde archivos pacientes
...
def guardar_en_archivoD():
    with open("doctores.txt", "w", encoding="utf-8") as archivo:
        for doctores in doctores_data:
            archivo.write(f"{doctores['Nombre']}|{doctores['Especialidad']}|{doctores['Edad']}|"
            f"{doctores['Telefono']}\n")   
...
def eliminardoctores():
    ...
    if seleccionado:
        indice=int(seleccionado[0])
        id_item=seleccionado[0]
        if messagebox.askyesno("Eliminar Doctor", f"¿Estas seguro de eliminar el doctor'{treeview.item(id_item,'values')[0]}'?"):
            del doctores_data[indice]
            guardar_en_archivoD()#Guardar cambios en el archivo
            cargar_treeview()
